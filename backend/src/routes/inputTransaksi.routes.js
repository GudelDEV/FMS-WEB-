...
import { upload } from "../middlewares/upload.js"; // kalau beda file tinggal sesuaikan
const router = express.Router();

// POST /api/transaksi — FIXED
router.post("/", upload.array("bukti"), async (req, res) => {
  try {
  ...
  try {
    ...
    if (existing.status !== "pending")
      return res.status(400).json({
        message: "Transaksi sudah di-approve/reject, tidak bisa diedit",
      });
